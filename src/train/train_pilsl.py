...
def train_pilsl(parameters, pos_samples, neg_samples, mode=None, save_mat=False, ex_compt=None,indep_test=None):
    if indep_test:
    ...
    for fold_num in range(kfold):
        ...
        if indep_test:
            valid_pos=np.hstack([valid_pos_kfold[fold_num], np.zeros((valid_pos_kfold[fold_num].shape[0], 1))])
            valid_neg=np.hstack([valid_neg_kfold[fold_num], np.zeros((valid_neg_kfold[fold_num].shape[0], 1))])
            
            test_pairs=np.vstack([train_neg,train_pos,valid_neg,valid_pos,test_neg,test_pos])
        ...
        if indep_test:
            ...
            if save_mat:
                if checktosave.update_classify(fold_num, 0, np.asarray([valid_metrics[0], valid_metrics[2], valid_metrics[1]])):
                    ...
                    if not os.path.exists(f'../results/{n_s}{base_suffix}/pilsl'):
                        os.makedirs(f'../results/{n_s}{base_suffix}/pilsl')
                    path = f'../results/{n_s}{base_suffix}/pilsl/pilsl_fold_{fold_num}_pos_neg_{p_n}_{d_s}_{n_s}_classify.npy'
                ...
                if checktosave.update_ranking(fold_num, 0, valid_metrics[3:]):
                    # print('Saving score matrix ...')
                    path = f'../results/{n_s}{base_suffix}/pilsl/pilsl_fold_{fold_num}_pos_neg_{p_n}_{d_s}_{n_s}_ranking.npy'
                    checktosave.save_mat(path, score_mat)

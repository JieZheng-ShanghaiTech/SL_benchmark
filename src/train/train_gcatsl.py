...
def train_gcatsl(parameters, pos_samples, neg_samples):
    ...
    for fold_num in range(kfold):
        ...
        if 'All' in n_s:
            n_s = n_s.split('_')[1]
        path_global = os.path.normpath(f'../data/precessed_data/interaction_global_{fold_num}_{d_s}_{n_s}_1000.txt')
        # path_global = os.path.normpath(f'/data/yimiaofeng/SLBench/Bench/data/gcatsl/interaction_global_{fold_num}_{d_s}_{n_s}_1000.txt')
        # path_global = os.path.normpath(f'/data/yimiaofeng/SLBench/Bench/data/gcatsl/interaction_global_{fold_num}_{int(1 / p_n)}_{d_s}_{n_s}_1000.txt')
                ...
                for epoch in range(nb_epochs):
                    ...
                    if checktosave.update_classify(fold_num, epoch, np.asarray([test_metrics[0], test_metrics[2], test_metrics[1]])):
                        ...
                        if not os.path.exists(f'../results/{n_s}_score_mats/gcatsl'):
                            os.mkdir(f'../results/{n_s}_score_mats/gcatsl')
